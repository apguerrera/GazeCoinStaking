  ...
  <div @click="closeModal()" class="overlay">
    <div @click="stopPropogation" class="modal">
      ...
      <div class="modal-body">
        ...
        <button @click="unstake" class="modal-body__button btn">
          Unstake
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async unstake() {
      ...
      console.log(unstakeValueInWei);

      const receipt = await unstakeLpSend("unstake", [unstakeValueInWei], {
        from: this.account,
        value: numberToHex(`${unstakeValueInWei}`)
      });
